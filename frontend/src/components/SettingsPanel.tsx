  ...
  return (
    <div className="h-full flex flex-col">
      ...
      <div className="flex-1 overflow-y-auto p-5 space-y-5">
            ...
            {typeof Notification !== 'undefined' && Notification.permission !== 'granted' && (
              <button
                onClick={() => Notification.requestPermission().then((perm) => {
                  if (perm === 'granted') applyInstant({ desktopNotifications: true });
                })}
                className="text-[9px] text-primary bg-primary/10 px-2 py-0.5 rounded-md hover:bg-primary/15 transition-colors"
              >
                Request Permission
              </button>
            )}
            <button
              onClick={() => applyInstant({ desktopNotifications: !settings.desktopNotifications })}
              className={`w-10 h-5 rounded-full relative transition-all ${
        ...
        {/* Quiet hours */}
        <SettingField label="Quiet Hours">
          <div className="flex items-center gap-3">
            <div className="flex items-center gap-1.5">
              <span className="text-[9px] text-on-surface-variant/40">From</span>
              <input
                type="number"
                min={0}
                max={23}
                value={display.quietHoursStart ?? 22}
                onChange={(e) => updateDraft({ quietHoursStart: Number(e.target.value) })}
                className="setting-input w-14 text-center text-[12px] py-1"
              />
              <span className="text-[9px] text-on-surface-variant/30">:00</span>
            </div>
            <div className="flex items-center gap-1.5">
              <span className="text-[9px] text-on-surface-variant/40">To</span>
              <input
                type="number"
                min={0}
                max={23}
                value={display.quietHoursEnd ?? 7}
        ...
        <div className="flex items-center justify-between py-1">
          <div>
            ...
            <p className="text-[9px] text-on-surface-variant/30 mt-0.5">
              Show extra logging and connection info
            </p>
          </div>
          <button
