          ...
          return (
            <div key={a.name} className="flex items-center gap-2 py-1">
              <div
                ...
                style={{ backgroundColor: isOn ? a.color : 'var(--color-outline-variant)', boxShadow: isOn ? `0 0 6px ${a.color}50` : 'none' }}
              />
              <span className="text-[11px] text-on-surface-variant flex-1 truncate">{a.label}</span>
              <span className={`text-[9px] font-semibold ${
                a.state === 'thinking' ? 'text-yellow-400' : isOn ? 'text-green-400/80' : 'text-on-surface-variant/40'
              }`}>
            ...
            return (
              <div key={sender} className="py-1">
                <div className="flex items-center justify-between mb-1">
                  <span className="text-[10px] text-on-surface-variant truncate">{agent?.label || sender}</span>
                  <span className="text-[10px] text-on-surface-variant/60 font-medium">{count}</span>
                </div>
...
function StatCard({ title, children }: { title: string; children: React.ReactNode }) {
  return (
    <div className="rounded-xl p-3 bg-surface-container/60 border border-outline-variant/8">
      <div className="text-[9px] font-semibold text-on-surface-variant/50 uppercase tracking-wider mb-2.5">
        {title}
      </div>
