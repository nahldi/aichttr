...
_INSTRUCTIONS = (
    ...
    "web_search(query, num_results) — Search the web via DuckDuckGo. Returns titles and URLs.\n"
    "web_fetch(url, max_length) — Fetch a URL and return text content (HTML auto-stripped).\n"
    "file_read(path, offset, limit) — Read a file with line numbers.\n"
    "file_write(path, content) — Write to a file (creates parent dirs).\n"
    "file_list(directory) — List directory contents.\n"
    "shell_exec(command, timeout) — Run a shell command (max 120s timeout).\n"
    "git_status() — Git porcelain status.\n"
    "git_diff() — Unified diff of uncommitted changes.\n"
    "git_log(n) — Last N commits in oneline format.\n"
    "system_info() — CPU, memory, disk, Python version.\n\n"
    "— Memory & Notes —\n"
    "memory_save(agent, key, value) — Save a key-value pair to agent-scoped persistent memory.\n"
    "memory_load(agent, key) — Load a value from memory.\n"
    "memory_list(agent) — List all memory keys.\n"
    "memory_search(agent, query) — Search memories by key and content.\n"
    "memory_delete(agent, key) — Delete a memory key.\n"
    "notes_save(agent, content) — Save freeform agent notes.\n"
    "notes_load(agent) — Load agent notes.\n"
    "soul_read(agent) — Read the agent's soul/personality config.\n\n"
    "— Agent Management —\n"
    "agent_spawn(base, label, task) — Spawn a worker agent sub-process (for manager agents).\n"
)
...
class _HTMLTextExtractor(HTMLParser):
    """Extracts visible text from HTML using only the stdlib."""
...
# ── New MCP tools — stdlib only ────────────────────────────────────

def web_search(query: str, num_results: int = 5, ctx: Context | None = None) -> str:
    """Search the web using DuckDuckGo HTML search. Returns titles + URLs.

    Args:
        query: Search query string
        num_results: Max results to return (default 5, max 20)
    """
    import urllib.request
    import urllib.parse
    import re
    ...
    req = urllib.request.Request(url, headers={"User-Agent": "Mozilla/5.0 (compatible; AIChattr/1.0)"})
    try:
...
def web_fetch(url: str, max_length: int = 8000, ctx: Context | None = None) -> str:
...
def file_read(path: str, offset: int = 0, limit: int = 200, ctx: Context | None = None) -> str:
    ...
    try:
        ...
        numbered = [f"{i+1:>5} | {lines[i]}" for i in range(offset, end)]
        header = f"File: {p} ({len(lines)} lines total, showing {offset+1}-{end})"
        return header + "\n" + "\n".join(numbered)
    except Exception as e:
        return f"Error: {e}"


def file_write(path: str, content: str, ctx: Context | None = None) -> str:
...
def file_list(directory: str = ".", ctx: Context | None = None) -> str:
    ...
    try:
        ...
        return f"{d}/\n" + "\n".join(entries) if entries else f"{d}/ (empty)"
    except Exception as e:
        return f"Error: {e}"


def shell_exec(command: str, timeout: int = 60, ctx: Context | None = None) -> str:
...
def git_status(ctx: Context | None = None) -> str:
    """Get git status (porcelain format) for the current directory."""
    import subprocess
    try:
        result = subprocess.run(
...
def git_diff(ctx: Context | None = None) -> str:
    """Get unified diff of uncommitted changes (truncated to 20k chars)."""
    import subprocess
    try:
        result = subprocess.run(
            ["git", "diff", "HEAD"],
...
def git_log(n: int = 10, ctx: Context | None = None) -> str:
    """Get last N git commits in oneline format.
...
def system_info(ctx: Context | None = None) -> str:
    """Get system information: CPU, memory, disk, Python version."""
    import platform
    import os
    ...
    # Memory info (cross-platform attempt)
    try:
        import shutil
        total, used, free = shutil.disk_usage("/")
        lines.append(f"Disk /: {used // (1024**3)}GB used / {total // (1024**3)}GB total ({free // (1024**3)}GB free)")
    except Exception:
        pass
...
def memory_save(agent: str, key: str, value: str, ctx: Context | None = None) -> str:
    """Save a key-value pair to agent-scoped persistent memory.
    ...
    mem_dir = _data_dir / "memories" / agent
    mem_dir.mkdir(parents=True, exist_ok=True)
    (mem_dir / f"{key}.txt").write_text(value, encoding="utf-8")
    return f"Saved memory '{key}' for {agent} ({len(value)} chars)"


def memory_load(agent: str, key: str, ctx: Context | None = None) -> str:
    """Load a value from agent-scoped persistent memory.
    ...
    if not _data_dir:
        return "Error: data directory not configured."
    path = _data_dir / "memories" / agent / f"{key}.txt"
    if not path.exists():
        return f"Error: no memory with key '{key}' for {agent}."
    return path.read_text(encoding="utf-8")


def memory_list(agent: str, ctx: Context | None = None) -> str:
    ...
    if not mem_dir.is_dir():
        return f"No memories for {agent}."
    keys = [f.stem for f in sorted(mem_dir.glob("*.txt"))]
    return json.dumps(keys) if keys else f"No memories for {agent}."


def memory_search(agent: str, query: str, ctx: Context | None = None) -> str:
    """Search through agent memories by key name and content.
    ...
    q = query.lower()
    results = []
    for f in sorted(mem_dir.glob("*.txt")):
        key = f.stem
        content = f.read_text(encoding="utf-8", errors="replace")
        if q in key.lower() or q in content.lower():
            preview = content[:100] + "..." if len(content) > 100 else content
            results.append({"key": key, "preview": preview})
    return json.dumps(results) if results else f"No memories matching '{query}' for {agent}."


def memory_delete(agent: str, key: str, ctx: Context | None = None) -> str:
    ...
    if not path.exists():
        return f"Error: no memory with key '{key}' for {agent}."
    path.unlink()
    return f"Deleted memory '{key}' for {agent}."


def notes_save(agent: str, content: str, ctx: Context | None = None) -> str:
    """Save agent notes (freeform text).
    ...
    (notes_dir / f"{agent}.md").write_text(content, encoding="utf-8")
    return f"Notes saved for {agent} ({len(content)} chars)"


def notes_load(agent: str, ctx: Context | None = None) -> str:
    """Load agent notes.
    ...
    if not _data_dir:
        return "Error: data directory not configured."
    path = _data_dir / "notes" / f"{agent}.md"
    if not path.exists():
        return f"No notes for {agent}."
    return path.read_text(encoding="utf-8")


def soul_read(agent: str, ctx: Context | None = None) -> str:
    """Read the soul/personality/system-prompt fragment for an agent.
    ...
    if not _data_dir:
        return "Error: data directory not configured."
    path = _data_dir / "souls" / f"{agent}.md"
    if not path.exists():
        return f"No soul configured for {agent}."
    return path.read_text(encoding="utf-8")
...
def agent_spawn(
    ...
    """Spawn a worker agent sub-process. Used by manager agents to delegate tasks.

    Args:
        base: Agent base name (e.g. 'claude', 'codex')
        label: Optional display label
        task: Task description to pass to the worker
    """
    import urllib.request
    ...
    # Register via HTTP to the main server
    payload = json.dumps({"base": base, "label": label or f"{base}-worker"})
    req = urllib.request.Request(
        f"http://127.0.0.1:{_server_port}/api/spawn-agent",
        data=payload.encode(),
        headers={"Content-Type": "application/json"},
        method="POST",
    )
    try:
        ...
        if result.get("ok"):
            msg = f"Spawned {base} worker (pid {result.get('pid', '?')})"
            if task:
                msg += f". Task: {task}"
            return msg
        return f"Error: {result.get('error', 'spawn failed')}"
    except Exception as e:
...
_ALL_TOOLS = [
    ...
    system_info,
    memory_save, memory_load, memory_list, memory_search, memory_delete,
    notes_save, notes_load,
    soul_read,
    agent_spawn,
]
